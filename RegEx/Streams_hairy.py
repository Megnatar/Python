...
def get_streams(url, users):
    ...
    oke = '200'
    i = 0
    if os.path.exists(save_dir + '/Streams_hairy_' + str(p) + '.m3u8'):
        os.remove(save_dir + '/Streams_hairy_' + str(p) + '.m3u8')
    Playlist = open(save_dir + '/Streams_hairy_' + str(p) + '.m3u8', encoding='utf-8', mode='a')
            ...
            if strm_url:
                ...
                if i == 90:
                    ...
                    if p <= pages:
                        ...
                        if os.path.exists(save_dir + '/Streams_hairy_' + str(p) + '.m3u8'):
                            os.remove(save_dir + '/Streams_hairy_' + str(p) + '.m3u8')
                        with open(save_dir + '/Streams_hairy_' + str(p) + '.m3u8', encoding='utf-8', mode='a') as Playlist:
                            Playlist.write('#EXTM3U\n') 
            else:
                unavailable_user -= 1
                continue
...
get_streams(url[0], get_users(url[4], 2))
