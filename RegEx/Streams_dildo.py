...
def get_streams(url, users):
    ...
    oke = '200'
    i = 0
    if os.path.exists(save_dir + '/Streams_dildo_' + str(p) + '.m3u8'):
        os.remove(save_dir + '/Streams_dildo_' + str(p) + '.m3u8')
    with open(save_dir + '/Streams_dildo_' + str(p) + '.m3u8', encoding='utf-8', mode='a') as Playlist:
...
get_streams(url[0], get_users(url[2], 2))
